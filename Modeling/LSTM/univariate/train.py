"""LSTM univariate — same data setup as MLP mask_baseline, ≥ 50 000 iterations."""
...
from model   import UnivariateLSTM
...
print(f"[LSTM]  device={device}  |  Window={WINDOW}")
...
print(f"Batches/epoch={batches_per_epoch}  |  Epochs={EPOCHS}  |  Total iter={EPOCHS*batches_per_epoch}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    with torch.no_grad():
        for X, m, y in test_loader:
            X, m, y = X.to(device), m.to(device), y.to(device)
            va += criterion(model(X, m), y).item() * len(y)
    va /= len(test_ds)
    ...
    if epoch == 1 or epoch % max(1, EPOCHS // 20) == 0:
        print(f"Epoch {epoch:>4d}/{EPOCHS} | train={tr:.5f} | val={va:.5f}")

print(f"\n[LSTM] Best val loss: {best_val:.5f}  (RMSE_scaled = {np.sqrt(best_val):.4f})")
...
ax.set_title(f"LSTM univariate — loss curve  (best val={best_val:.4f})")
ax.grid(True, alpha=0.3);  ax.legend()
plt.tight_layout()
plt.savefig(PLOTS_DIR / "01_loss_curve.png", dpi=130);  plt.close()

# Predicted vs actual on first 3 test engines
...
for ax, eid in zip(axes, unique_engines):
    ...
    for t in idx[:-1]:
        ...
        if start < 0:
            ...
            wv = series[start:t + 1];  wm = mask[start:t + 1]
        x_t = torch.tensor(wv).unsqueeze(0).to(device)
        m_t = torch.tensor(wm).unsqueeze(0).to(device)
        with torch.no_grad():
            preds.append(model(x_t, m_t).item())
        actuals.append(series[t + 1])
    ax.plot(actuals, label="actual",    color="steelblue", linewidth=1.5)
    ax.plot(preds,   label="LSTM pred", color="tomato",    linewidth=1.2, alpha=0.9)
    ax.set_title(f"LSTM — Engine {eid}")
