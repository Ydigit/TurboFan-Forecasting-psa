...
print(f"[MASK]  Device: {device}  |  Window: {WINDOW}")
...
print(f"\n[MASK] Best val loss: {best_val:.5f}  (RMSE scaled = {np.sqrt(best_val):.4f})")
...
ax.set_title(f"Loss curve — WITH mask  (best val={best_val:.4f})")
...
for ax, eid in zip(axes, unique_engines):
    ...
    for t in idx[:-1]:
        ...
        if start < 0:
            ...
            win_vals = series[start:t + 1];  win_mask = mask[start:t + 1]
        x_t = torch.tensor(win_vals).unsqueeze(0).to(device)
        m_t = torch.tensor(win_mask).unsqueeze(0).to(device)
        with torch.no_grad():
    ...
    ax.set_title(f"WITH mask — Engine {eid}")
