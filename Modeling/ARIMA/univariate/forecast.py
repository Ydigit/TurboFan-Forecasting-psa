"""
ARIMA(1,1,1) — 1-step-ahead forecast for sensor_11.

Per-engine rolling-origin forecast on the test set, mirroring the MLP evaluation:
  for each test engine, for each cycle t (>= WINDOW), fit ARIMA on history[0..t]
  and forecast t+1.

Order (1,1,1) chosen because:
  - d=1 makes the series stationary (sensor_11 has a clear trend)
  - p=1 captures short-term autoregressive dependence
  - q=1 captures short-term MA component (residual autocorrelation)
...
WINDOW = 10
ORDER  = (0, 1, 1)   # (p, d, q) — chosen by BIC + AIC top-3 in grid search
...
for ax, eid in zip(axes, unique_engines):
    ...
    ax.plot(preds[idx],   label=f"ARIMA{ORDER}", color="tomato",   linewidth=1.2, alpha=0.9)
    ax.set_title(f"Engine {eid} — ARIMA{ORDER} 1-step-ahead")
...
ax.set_title(f"ARIMA{ORDER} residuals — RMSE={rmse_scaled*scale:.4f} psia, R²={r2:.4f}")
